...
class LoginController extends GetxController {
  ...
  login({context}) async {
    ...
    try {
      Map data = {
        'mixed': emailController.text.toString().trim(),
        'password': passwordController.text.toString().trim(),
      };
      var response = await networkRepository.login(data, context);

      if (response != null && response['status'] == "success") {
        dataStorage.write('token', response['token']);
        dataStorage.write('balance', response['balance']);
        dataStorage.write('username', response['username']);
        // await Purchases.logIn(dataStorage.read('username'));
        dataStorage.write('email', response['email']);
        dataStorage.write('first_name', response['first_name']);
        dataStorage.write('last_name', response['last_name']);
        dataStorage.write(
        ...
        isLoading = false;
        update();
        // await getUploadToken(context: context);
      } else {
